...
class ConanFile(_ConanFile):
    ...
    def f0cal_yml(self):
        return {
            ...
            'admin_user': self.conan_data["admin_user"],
            'admin_password': self.conan_data["admin_password"],
            'parts': self.conan_data["parts"],
        }
    ...
    def additional_urls(self):
        ...
        :return: URL
        """
        return self.conan_data["additional_urls"]
    ...
    def base_from_paths(self) -> bool:
        ...
        if base_path:
            ...
            for base_directory in base_paths:
                if get_file_from_path(base_directory, self.output_filename, self.conan_data["output_hash"]):
                    found_base = True
                    break
                if (
    ...
    def download_base(self):
        ...
        for url in [self.url, *self.additional_urls]:
            ...
            if (
                    ...
                    and
                    assert_file_sha1(f'{self.download_filename}.part', self.conan_data["download_hash"])
            ):
                success = True
                break
        if not success:
    ...
    def build(self):
        ...
        assert_file_sha1(self.output_filename, self.conan_data["output_hash"])
